...
const natives = [...module_1.builtinModules, 'bootstrap_node', 'node'].map((n) => new RegExp(`(?:\\((?:node:)?${n}(?:\\.js)?:\\d+:\\d+\\)$|^\\s*at (?:node:)?${n}(?:\\.js)?:\\d+:\\d+$)`));
...
class StackUtils {
     ...
     */
    capture(limit = Infinity, startStackFunction = this.capture) {
        if (typeof limit == 'function') {
    ...
    ignoredPackagesRegExp(ignoredPackages) {
